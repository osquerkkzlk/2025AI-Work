...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Monet--->Photo"):
        with gr.Row(equal_height=True):
            A_image_input=gr.Image(label="原始莫奈风格的图像")
            A_image_output=gr.Image(label="转换后的图像",interactive=False)
    ...
    B_convert_button.click(AppTransform_Photo2Monet,inputs=[A_image_input],outputs=[A_image_output],
                           )
